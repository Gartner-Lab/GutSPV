...
def plot_graph(graph_mat, state_mat, output_path, pos=None):
    """
    Visualizes the adjacency graph with node colors and opacity based on state matrix.
    Saves the output as a PNG file.
    Ensures consistent layout for movie generation by allowing a fixed `pos` argument.
    """
    ...
    # Use the provided layout or calculate one if not provided
    if pos is None:
    ...
    return pos  # Return the layout to reuse it for subsequent frames
        ...
        for j in range(graph_mat.shape[1]):
            if graph_mat[i, j] == 1:  # Check if there is an edge
                ...
                if type_i == type_j:
                    ...
                    color_coded_mat[i, j] = 3  # Blue for Type 10
